# Adventure Game
import os
os.environ['SDL_VIDEODRIVER'] = 'windows'  # Use native macOS window system
...
# Testing if model access is granted

client = OpenAI()  # it will auto-load API key from env OPENAI_API_KEY
try:
...
print(f"Pygame init: {successes} successes and {failures} failures")
display = (800, 600)
init_results = {
    "pygame.display": pygame.display.get_init(),
    "pygame.font": pygame.font.get_init(),
    "pygame.mixer": pygame.mixer.get_init(),
    "pygame.joystick": pygame.joystick.get_init(),
    "pygame.image": pygame.image.get_extended(),
}

for module, status in init_results.items():
    print(f"{module} initialized: {status}")
...
class DialogueSystem:
    ...
    def send_message(self):
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-4.1",  # or your current model
...
class Game3D:
    ...
    def __init__(self):
        ...
        # base.win_props = base.win.get_properties()
